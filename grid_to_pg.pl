...
my $hbs_id = 'igow';my $dbname = $PGDATABASE;
my $fix_missing = '';
my $updated = '';
my $drop = '';
my $db_schema = 'public';
...
if ($drop ne '') {
  $table_name = "$table_name(drop=$drop)";
}
...
$cmd = "echo \"$sas_code\" | ";
$cmd .= "ssh -C $hbs_id\@researchgrid.hbs.edu 'sas  -stdio -noterminal ' 2>/dev/null";
print "$cmd\n";
$result = `$cmd`;
